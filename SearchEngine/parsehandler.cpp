...
void ParseHandler::parse(string inputFile){
    ...
    xml_node<> * media = doc.first_node();
    cout << "..." << endl;

    // Find our page node. Iterate over all the pages
    for(page_node = media->first_node("page"); page_node; page_node = page_node->next_sibling()){
        pageCount++;

        xml_node<>* title_node = page_node->first_node("title");
        if (title_node == 0)
            title = "[No page title recorded]";
        else
            title = title_node->value();
        xml_node<>* date_node = page_node->first_node("revision")->first_node("timestamp");
        if (date_node == 0)
            date = "[No page date recorded]";
        else
            date = date_node->value();
        xml_node<>* author_node = page_node->first_node("revision")->first_node("contributor")->first_node("username");
        if (author_node == 0)
            author = "[No page author recorded]";
        else
            author = author_node->value();

        id_node = page_node->first_node("id");
        id = atoi(id_node->value()); //grab id
        index->addDocId(id);

        for(id_node = id_node->next_sibling(); id_node; id_node = id_node->next_sibling()){
            text_node = id_node->first_node("text");
            if(text_node){
                text = text_node->value();

                docIO->addDocument(id,title,author,date,text);

                transform(text.begin(), text.end(), text.begin(), ::tolower);
                c_text = new char[text.length() + 1] ;
                strcpy(c_text, text.c_str());
                token = strtok(c_text, delimiters);
                while(token != NULL){
                    //Remove Stopwords
                    if (stopWords->count(token) == 0)
                    {
                        unordered_map<string,string>::const_iterator stemmed = stemWords->find(token);
                        if (stemmed == stemWords->end())
                        {
                            string copy = token;
                            Porter2Stemmer::stem(copy);
                            stemWords->insert({token,copy});
                            index->insert(copy);
                        }
                        else
                        {
                            string word(stemmed->second);
                            index->insert(word);
                        }
                    }
